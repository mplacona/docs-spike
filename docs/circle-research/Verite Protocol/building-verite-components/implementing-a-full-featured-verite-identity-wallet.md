...
slug: "implementing-a-full-featured-verite-identity-wallet"
excerpt: "Verite Wallet Integration Guide"
hidden: false
...
See the [list of specifications and spec-conforming libraries](doc:specifications-and-libraries) used by Verite.

## Credential Flows

The credential flows demonstrate use of a non-custodial credential and identifier wallet, but are easily adapted to hosted wallets, combination crypto+identity wallets, and various variations on the described flow. See the wallet-bound variants of the [issuance](doc:issuing-credentials#issuance-flow) and [verification](doc:verifying-credentials#verification-flow) flow sections elsewhere in the docs for context.
...
4. On approval, the wallet generates/provides a decentralized identifier, builds the credential request, and signs it to achieve [proof of identifier control](https://identity.foundation/presentation-exchange/#proof-of-identifier-control).
   ...
   2. The Verite library exposes a convenience method `createCredentialApplication` for this purpose -- this is used by demo-wallet in preparing a credential application.
   3. Note: the Verite demo does not require the wallet to submit credentials for issuance, as the credential recipient is already known to the issuer, but the credential manifest and credential application structures allow for this if needed.
   4. Example: [Credential Application](#credential-application)
...
6. The issuer issues the credential and returns it to the wallet
   ...
   2. In the Verite demo, the issuer replies with a [credential fulfillment](https://identity.foundation/credential-manifest/#credential-fulfillment)-structured result containing a JWT-encoded Verifiable Presentation.
   3. Examples:
      1. [Credential Fulfillment](#credential-fulfillment)
      2. [Decoded Credential](#decoded-credential)
...
![Credential exchange sequence diagram demonstrating the steps described below](https://files.readme.io/27eba8f-sequence_issuance.png)
